...
// ─── Template 5: Calculator Results (Client copy) ────────────────────────────
// results shape varies by calculator type — see below
export function calculatorResultsClient({ name, calculatorName, resultsHtml }) {
  const firstName = name?.split(' ')[0] || 'there'
  const subject = `Your ${calculatorName} results from Performuscle`

  const html = wrap(`
    <h1 style="margin:0 0 6px;font-family:'Bebas Neue',Impact,'Arial Narrow',sans-serif;font-size:26px;letter-spacing:0.08em;color:${BRAND.text};">
      Your ${calculatorName} Results
    </h1>
    <p style="margin:0 0 28px;font-size:15px;color:${BRAND.muted};line-height:1.6;">
      Hey <strong style="color:${BRAND.text};">${firstName}</strong> — here's a summary of your results, ready to save or share with your coach.
    </p>

    ${resultsHtml}

    <div style="margin:28px 0 0;padding:16px 20px;background:#f8f9fb;border-radius:10px;border-left:3px solid ${BRAND.accent};">
      <p style="margin:0;font-size:13px;color:${BRAND.muted};line-height:1.6;">
        These results were generated by the Performuscle coach toolkit. For a personalised plan based on these numbers, reach out to a Performuscle coach.
      </p>
    </div>
  `)
...
// ─── Template 6: Calculator Results (Coach copy) ─────────────────────────────
export function calculatorResultsCoach({ name, email, calculatorName, resultsHtml }) {
  const subject = `${name} just used the ${calculatorName}`
  ...
  const html = wrap(`
    ...
    <p style="margin:0 0 24px;font-size:15px;color:${BRAND.muted};line-height:1.6;">
      <strong style="color:${BRAND.text};">${name}</strong> (<a href="mailto:${email}" style="color:${BRAND.accent};">${email}</a>) used the <strong style="color:${BRAND.text};">${calculatorName}</strong> on your coach tools page.
    </p>

    ${resultsHtml}

    <p style="margin:24px 0 0;font-size:12px;color:${BRAND.muted};text-align:center;line-height:1.6;">
      Reply to this email or reach out at ${email} to follow up.
    </p>
  `)
...
// ─── Shared: results HTML builders ───────────────────────────────────────────

function statRow(label, value, accent = false) {
  return `
    <tr>
      <td style="padding:10px 12px;border-bottom:1px solid #eef0f4;font-size:13px;color:${BRAND.muted};width:55%;">${label}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #eef0f4;font-size:14px;font-weight:600;color:${accent ? BRAND.accent : BRAND.text};text-align:right;">${value}</td>
    </tr>`
}

export function build1rmResultsHtml({ exercise, weight, reps, unit, estimated1rm, formulas }) {
  const exerciseLine = exercise
    ? `<p style="margin:0 0 16px;font-family:'Bebas Neue',Impact,'Arial Narrow',sans-serif;font-size:18px;letter-spacing:.06em;color:${BRAND.text};">${exercise.toUpperCase()}</p>`
    : ''

  const bigStat = `
    <div style="text-align:center;padding:24px 0 20px;border-bottom:1px solid #eef0f4;margin-bottom:20px;">
      <div style="font-family:'Bebas Neue',Impact,'Arial Narrow',sans-serif;font-size:11px;letter-spacing:.12em;color:${BRAND.muted};margin-bottom:6px;">ESTIMATED 1 REP MAX</div>
      <div style="font-family:'Bebas Neue',Impact,'Arial Narrow',sans-serif;font-size:56px;letter-spacing:.02em;color:${BRAND.accent};line-height:1;">${estimated1rm}</div>
      <div style="font-family:'Bebas Neue',Impact,'Arial Narrow',sans-serif;font-size:20px;letter-spacing:.06em;color:${BRAND.muted};">${unit.toUpperCase()}</div>
      <div style="font-size:12px;color:${BRAND.muted};margin-top:6px;">Average across all formulas</div>
    </div>`

  const formulaRows = formulas.map(f => statRow(f.name, `${f.value} ${unit}`)).join('')

  return `
    <div style="background:#f8f9fb;border-radius:10px;padding:20px 24px;margin-bottom:16px;">
      ${exerciseLine}
      ${bigStat}
      <div style="font-family:'Bebas Neue',Impact,'Arial Narrow',sans-serif;font-size:11px;letter-spacing:.12em;color:${BRAND.muted};margin-bottom:8px;">INPUT</div>
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;margin-bottom:20px;">
        ${statRow('Weight lifted', `${weight} ${unit}`)}
        ${statRow('Reps performed', reps)}
      </table>
      <div style="font-family:'Bebas Neue',Impact,'Arial Narrow',sans-serif;font-size:11px;letter-spacing:.12em;color:${BRAND.muted};margin-bottom:8px;">FORMULA BREAKDOWN</div>
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
        ${formulaRows}
        ${statRow('Average (recommended)', `${estimated1rm} ${unit}`, true)}
      </table>
    </div>`
}

export function buildBfResultsHtml({ gender, bf, category, colorClass }) {
  const color = colorClass === 'color-green' ? BRAND.accent
    : colorClass === 'color-amber' ? BRAND.warn
    : BRAND.danger

  return `
    <div style="background:#f8f9fb;border-radius:10px;padding:20px 24px;margin-bottom:16px;text-align:center;">
      <div style="font-family:'Bebas Neue',Impact,'Arial Narrow',sans-serif;font-size:11px;letter-spacing:.12em;color:${BRAND.muted};margin-bottom:10px;">BODY FAT PERCENTAGE (NAVAL METHOD)</div>
      <div style="font-family:'Bebas Neue',Impact,'Arial Narrow',sans-serif;font-size:60px;letter-spacing:.02em;color:${color};line-height:1;">${bf}%</div>
      <div style="font-family:'Bebas Neue',Impact,'Arial Narrow',sans-serif;font-size:18px;letter-spacing:.1em;color:${color};margin-top:6px;">${category}</div>
      <div style="font-size:12px;color:${BRAND.muted};margin-top:8px;">${gender === 'male' ? 'Male' : 'Female'} · ACE body fat categories</div>
    </div>`
}

export function buildRolResultsHtml({ rateLabel, rate, weeklyLoss, unit, dailyDeficit, targetCals, totalLoss, weeks, months, targetDate, fatLost, leanLost, finalWeight, finalBf }) {
  return `
    <div style="background:#f8f9fb;border-radius:10px;padding:20px 24px;margin-bottom:16px;">
      <div style="font-family:'Bebas Neue',Impact,'Arial Narrow',sans-serif;font-size:11px;letter-spacing:.12em;color:${BRAND.muted};margin-bottom:14px;">RATE OF LOSS — ${rateLabel.toUpperCase()} (${rate}% BW/week)</div>
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
        ${statRow('Weekly loss target', `${weeklyLoss} ${unit}`)}
        ${statRow('Daily calorie deficit', `${dailyDeficit} kcal`)}
        ${statRow('Target daily calories', `${targetCals} kcal`, true)}
        ${statRow('Total weight to lose', `${totalLoss} ${unit}`)}
        ${statRow('Duration', `${weeks} weeks (${months} mo)`)}
        ${statRow('Estimated target date', targetDate, true)}
        ${statRow('Fat mass lost', `${fatLost} ${unit}`)}
        ${statRow('Lean mass lost', `${leanLost} ${unit}`)}
        ${statRow('Final weight', `${finalWeight} ${unit}`, true)}
        ${statRow('Final body fat', `${finalBf}%`, true)}
      </table>
    </div>`
}

export function buildStructuralBalanceResultsHtml({ deficits, totalDeficits, criticalCount, prioritySystem, startPhase }) {
  const deficitRows = deficits.length === 0
    ? `<tr><td colspan="3" style="padding:14px 12px;font-size:13px;color:${BRAND.accent};text-align:center;">All ratios within target range</td></tr>`
    : deficits.map(d => `
        <tr>
          <td style="padding:10px 12px;border-bottom:1px solid #eef0f4;font-size:13px;color:${BRAND.text};">${d.label}</td>
          <td style="padding:10px 12px;border-bottom:1px solid #eef0f4;font-size:13px;color:${BRAND.text};text-align:center;">${d.actual}%</td>
          <td style="padding:10px 12px;border-bottom:1px solid #eef0f4;font-size:13px;color:${d.tier === 'critical' || d.tier === 'deficit' ? BRAND.danger : BRAND.warn};text-align:center;">${d.target}%</td>
        </tr>`).join('')

  return `
    <div style="background:#f8f9fb;border-radius:10px;padding:20px 24px;margin-bottom:16px;">
      <div style="font-family:'Bebas Neue',Impact,'Arial Narrow',sans-serif;font-size:11px;letter-spacing:.12em;color:${BRAND.muted};margin-bottom:14px;">STRUCTURAL BALANCE SUMMARY</div>
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;margin-bottom:20px;">
        ${statRow('Total deficits', totalDeficits, totalDeficits > 0)}
        ${statRow('Critical flags', criticalCount, criticalCount > 0)}
        ${statRow('Priority system', prioritySystem || 'None')}
        ${statRow('Recommended start phase', startPhase)}
      </table>
      <div style="font-family:'Bebas Neue',Impact,'Arial Narrow',sans-serif;font-size:11px;letter-spacing:.12em;color:${BRAND.muted};margin-bottom:8px;">DEFICIT DETAIL</div>
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">
        <tr>
          <th style="text-align:left;font-size:10px;font-weight:700;color:${BRAND.muted};padding:6px 12px;letter-spacing:.08em;border-bottom:2px solid #eef0f4;">MOVEMENT</th>
          <th style="text-align:center;font-size:10px;font-weight:700;color:${BRAND.muted};padding:6px 12px;letter-spacing:.08em;border-bottom:2px solid #eef0f4;">ACTUAL</th>
          <th style="text-align:center;font-size:10px;font-weight:700;color:${BRAND.muted};padding:6px 12px;letter-spacing:.08em;border-bottom:2px solid #eef0f4;">TARGET</th>
        </tr>
        ${deficitRows}
      </table>
    </div>`
